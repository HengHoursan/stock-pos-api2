  ...
  async findAllWithPagination(
    ...
    if (filter) {
      if (filter.status && filter.status !== 'all') {
        queryBuilder.andWhere('sale_return.status = :status', {
          status: Number(filter.status),
        });
      }
      if (filter.customerId) {
        queryBuilder.andWhere('sale_return.customerId = :customerId', {
      ...
      if (filter.startDate) {
        queryBuilder.andWhere('sale_return.returnDate >= :startDate', {
      ...
      if (filter.endDate) {
        ...
        queryBuilder.andWhere('sale_return.returnDate <= :endDate', {
          endDate,
        });
      }
      ...
      if (filter.saleInvoiceId) {
        queryBuilder.andWhere('sale_return.saleInvoiceId = :saleInvoiceId', {
          saleInvoiceId: Number(filter.saleInvoiceId),
        });
      }
    }
