...
export class PurchasePaymentService {
  ...
  async create(
    ...
    if (existingCode) {
      throw new ConflictException(
        `Purchase Payment with code "${code}" already exists`,
      );
    }
  ...
  async findAllWithPagination(
    ...
    const [data, total] =
      await this.purchasePaymentRepository.findAllWithPagination(pagination);
  ...
  async update(
    ...
    if (dto.code && dto.code !== payment.code) {
      const existing = await this.purchasePaymentRepository.findByCode(
        dto.code,
      );
      if (existing) {
        throw new ConflictException(
          `Purchase Payment with code "${dto.code}" already exists`,
        );
      }
    }
      ...
      if (dto.details) {
        ...
        for (const oldDetail of payment.details) {
          if (oldDetail.purchaseInvoiceId) {
            const invoice = await manager.findOne(PurchaseInvoice, {
              where: { id: oldDetail.purchaseInvoiceId },
            });
            if (invoice) {
        ...
        await manager.delete(PurchasePaymentDetail, {
